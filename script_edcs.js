...
function buildAuthorizedFileLink(fileId, source) {
  const fileUrl = `${CONFIG.BASE_URL}/v1/files/${fileId}/content`;
  const encodedUrl = encodeURIComponent(fileUrl);
  const encodedName = encodeURIComponent(source);

  return `<a href="#" data-file-url="${encodedUrl}" data-file-name="${encodedName}" data-file-id="${fileId}" class="source-file-link">${source}</a>`;
}

async function handleAuthorizedFileDownload(event) {
  const link = event.target.closest(".source-file-link");
  if (!link) return;

  event.preventDefault();

  const fileUrl = decodeURIComponent(link.dataset.fileUrl || "");
  const fileName = decodeURIComponent(
    link.dataset.fileName || "downloaded-file",
  );
    ...
    if (sources.length > 0) {
      try {
        const uniqueSources = Array.from(
          new Map(
            (sources[0].metadata || [])
              .filter((item) => item?.file_id && item?.source)
              .map((item) => [item.file_id, item.source]),
          ).entries(),
        );
        ...
        const formattedSources = uniqueSources
          .map(
            ([fileId, source], idx) =>
              `+ [${idx + 1}] ${buildAuthorizedFileLink(fileId, source)}`,
          )
          .join("\n");
...
chatBox.addEventListener("click", handleAuthorizedFileDownload);
